...
router.get("/ suitableforkids/:status", (req, res) => {
    dog.find({  suitableForKids: req.params.status }) 
        .then(data => { 
...
router.get("/ suitableforapartment/:status", (req, res) => {
    dog.find({  suitableForApartment: req.params.status }) 
        .then(data => { 
            res.send(mapArray(data));  
        })
        .catch(err => {
...
router.put("/:id", (req, res) => {
    ...
    dog.findByIdAndUpdate(id, req.body)
        .then(data => {
            if (!data) {
                res.status(404).send({ message: "Cannot update this dog post with id=" + id + ". Maybe the post was not found!" });
            }
            else {
                res.send({ message: "Dog post was successfully updated." });
            }
        })
        .catch(err => {
            res.status(500).send({ message: "Error updating dog post with id=" + id })
        })
});
...
router.delete("/:id",  (req, res) => {
    ...
    dog.findByIdAndDelete(id)
        .then(data => {
            if (!data) {
                res.status(404).send({ message: "Cannot delete this dog post with id=" + id + ". Maybe the dog post was not found!" });
            }
            else {
        ...
        .catch(err => {
            console.debug(err);
            res.status(500).send({ message: "Error deleting dog post with id=" + id })
        })
});
